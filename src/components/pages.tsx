    ...
    <View style={styles.homePageStack}>
      <View style={[styles.heroGrid, !isWide && styles.stackedGrid]}>
        <View style={[styles.heroPanel, { borderColor: colors.borderColor, backgroundColor: colors.backgroundColor || colors.buttonBg }]}>
          ...
          <Text style={[styles.heroTitle, { color: colors.primaryText }]}>Production-ready vocabulary learning for students, exams, and everyday curiosity.</Text>
          <Text style={[styles.heroCopy, { color: colors.secondaryText }]}>GrabVocab combines dictionary lookup, curated academic word sets, subject and grade filtering, quizzes, and word-of-the-day discovery in one frontend backed by your live MongoDB content.</Text>
        </View>
...
export function WordPage({
  ...
  return (
    <View style={[styles.wordDetailCard, { borderColor: colors.borderColor, backgroundColor: colors.backgroundColor }]}> 
      ...
      {words.length > 0 && (
        <View style={[styles.paginationBar, { borderColor: colors.borderColor }]}> 
          ...
          <View style={styles.paginationActions}>
            ...
            <Text style={[styles.paginationText, { color: colors.primaryText }]}>{page} / {totalPages}</Text>
...
export function SubjectWordCard({
  ...
  return (
    <Pressable onPress={() => onSpeak(word.word)} style={[styles.subjectCard, { borderColor: colors.borderColor, backgroundColor: colors.backgroundColor || colors.buttonBg }]}> 
      <View style={styles.wordTitleRow}>
...
export function AboutPage({ colors }: { colors: ThemeColors }) {
  return (
    <View style={styles.textPage}>
      ...
      <Text style={[styles.aboutParagraph, { color: colors.primaryText }]}>At Grab Vocab, we believe learning should be smart, visual, and enjoyable. That&apos;s why we blend beautifully crafted word definitions, example sentences, memory tricks, and quizzes to help you remember and apply what you learn.</Text>
      <Text style={[styles.aboutParagraph, { color: colors.primaryText }]}>Whether you&apos;re preparing for competitive exams, enhancing your academic language, or just feeding your curiosity, Grab Vocab adapts to your journey.</Text>
    </View>
  );
}
...
export function QuizPage({
  ...
  const { width } = useWindowDimensions();
  const isWide = width >= 900;
  const subjects = ['english-literature', 'history', 'geography', 'biology', 'chemistry', 'physics', 'political-science', 'sociology', 'psychology'];
        ...
        <View style={[styles.quizCard, { borderColor: colors.borderColor, backgroundColor: colors.backgroundColor || colors.buttonBg }]}> 
          ...
          {subjects.map((subject) => (
            <Pressable
              ...
              style={[styles.secondaryActionButton, styles.quizAction, { borderColor: colors.borderColor }]}
            >
...
export function AuthPage({
  ...
  const googleEnabled = Boolean(GOOGLE_WEB_CLIENT_ID || GOOGLE_ANDROID_CLIENT_ID || GOOGLE_IOS_CLIENT_ID);
    ...
    <View style={[styles.contentGrid, styles.authGrid, !isWide && styles.stackedGrid]}>
      ...
      <View style={[styles.authCard, { borderColor: colors.borderColor, backgroundColor: colors.backgroundColor || colors.buttonBg }]}> 
        <View style={styles.pageStack}>
          ...
          <Pressable
            style={[styles.secondaryActionButton, styles.authSwitchButton, { borderColor: colors.borderColor }]}
...
export function InfoBlock({ title, value, colors }: { title: string; value: string; colors: ThemeColors }) {
  return (
